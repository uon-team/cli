...
export class ViewCompilerContext {
        ...
        function ViewTransformFactory(context: ts.TransformationContext) {
            ...
            function ViewVisitor(node: ts.Node): ts.Node {
                ...
                if (ts.isSourceFile(node)) {
                    ...
                    return ts.updateSourceFileNode(src_node,
                ...
                if (ts.isClassDeclaration(node)) {
                    ...
                    if (has_view || has_dir) {
                        ...
                        const sel_var = CreateSelectorDTSVariable(
                            class_name,
                        ...
                        let host_stmt = ts.createVariableDeclaration(
                            class_name + '.' + 'ʘH',
                            undefined,
                        ...
                        if (view_providers) {
                            let prov_stmt = ts.createVariableDeclaration(
                                class_name + '.' + 'ʘP',
                                undefined,
                                view_providers
                            );
                        ...
                        if (has_view) {
                            ...
                            let factory_stmt = ts.createVariableDeclaration(
                                class_name + '.' + 'ʘF',
                                undefined,
                                func
                            );
                            let selector_stmt = ts.createVariableDeclaration(
                                class_name + '.' + 'ʘT',
                                undefined,
                                ts.createStringLiteral(element_selector.element)
                            );
...
function CreateSelectorDTSVariable(className: string, prefix: string, type: string) {
        ...
        ts.createVariableDeclarationList(
            [
                ts.createVariableDeclaration(
                    `${prefix}${className}`,
                    ts.createLiteralTypeNode(ts.createLiteral(type)),
                ),
            ],
            ts.NodeFlags.Const,
        ),
